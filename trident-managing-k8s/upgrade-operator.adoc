...
NOTE: Pour mettre à niveau Trident installé à l'aide de l'opérateur à portée d'espace de noms (versions 20.07 à 20.10), utilisez les instructions de mise à niveau pour votre version installée de Trident.
...
.Étapes
. Si vous link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm["Installez Trident à l'aide de Helm - effectué"] , vous pouvez utiliser  `helm upgrade trident netapp-trident/trident-operator --version 100.2410.0` pour effectuer la mise à niveau en une seule étape. Si vous n'avez pas ajouté le dépôt Helm ou si vous ne pouvez pas l'utiliser pour effectuer la mise à niveau :
+
...
NOTE: Avant de changer de méthode d'installation, consultez link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["Passage d'une méthode d'installation à l'autre"].
