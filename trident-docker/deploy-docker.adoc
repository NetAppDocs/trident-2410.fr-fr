...
+
**  `config`: Le nom de fichier par défaut est `config.json`, mais vous pouvez utiliser n'importe quel nom en spécifiant l' `config`option avec le nom de fichier. Le fichier de configuration doit se trouver dans le `/etc/netappdvp` répertoire du système hôte.
...
Un exemple de fichier d'unité pour les systèmes basés sur le système se trouve `contrib/trident.service.example` dans le Git repo. Pour utiliser le fichier avec RHEL, procédez comme suit :
...
. Activer le service.
+
Ce nom varie en fonction de ce que vous avez appelé le fichier dans le `/usr/lib/systemd/system` répertoire.
...
NOTE: Chaque fois que vous modifiez le fichier d'unité, exécutez la `systemctl daemon-reload` commande pour qu'il soit informé des modifications.
