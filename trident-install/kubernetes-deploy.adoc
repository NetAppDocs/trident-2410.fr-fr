...
* Accès complet à un cluster Kubernetes compatible, exécutant une version compatible de Kubernetes et dont les fonctionnalités requises sont activées. Consultez la link:/trident-get-started/requirements.html["de formation"] pour plus de détails.
...
Que ce soit par déploiement manuel ou via Helm, l’opérateur Trident est un excellent moyen de simplifier l’installation et de gérer dynamiquement les ressources Trident. Vous pouvez même link:../trident-install/kubernetes-customize-deploy.html["Personnalisez le déploiement de l'opérateur Trident"] en utilisant les attributs de la  `TridentOrchestrator` ressource personnalisée (CR).
