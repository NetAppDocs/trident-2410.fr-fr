...
.Avant de commencer
En plus de link:../trident-install/kubernetes-deploy.html#before-you-deploy["conditions préalables au déploiement"] vous avez besoin de link:https://v3.helm.sh/["Version 3 de Helm"^].
...
----
. Utilisez `helm install` et spécifiez un nom pour votre déploiement comme dans l'exemple suivant où `100.2410.0` est la version de Trident que vous installez.
+
[listing]
----
