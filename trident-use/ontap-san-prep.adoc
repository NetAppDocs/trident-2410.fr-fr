...
summary: Découvrez comment vous préparer à configurer un back-end ONTAP avec les pilotes SAN ONTAP. 
---
...
Tous vos nœuds workers Kubernetes doivent avoir installé les outils iSCSI appropriés. Voir link:worker-node-prep.html["Préparez le nœud de travail"] pour plus de détails.
...
Vous pouvez mettre à jour les informations d'identification CHAP en mettant à jour les paramètres CHAP dans `backend.json` le fichier. Ceci nécessitera la mise à jour des secrets CHAP et l'utilisation de la `tridentctl update` commande pour refléter ces modifications.
