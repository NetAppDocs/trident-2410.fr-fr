...
Trident peut créer des volumes Cloud Volumes Service dans l'un des deux link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["types de service"^] :

* *CVS-Performance* : le type de service Trident par défaut. Ce type de service optimisé pour les performances convient le mieux aux charges de travail de production qui privilégient la performance. Le type de service CVS-Performance est une option matérielle prenant en charge les volumes d'une taille minimale de 100 Gio. Vous pouvez choisir l'une des options suivanteslink:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["trois niveaux de service"^] :
+
** `standard`
** `premium`
** `extreme`
...
* *CVS* : Le service CVS offre une haute disponibilité zonale avec des performances limitées à modérées. Le type de service CVS est une option logicielle qui utilise des pools de stockage pour prendre en charge des volumes d’une taille minimale de 1 Gio. Le pool de stockage peut contenir jusqu’à 50 volumes, où tous les volumes partagent la capacité et les performances du pool. Vous pouvez choisir l'une des options suivanteslink:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["deux niveaux de service"^] :
+
** `standardsw`
...
Pour configurer et utiliser le  https://www.netapp.com/google-cloud/netapp-volumes/["Cloud Volumes Service pour Google Cloud"^] backend, vous avez besoin des éléments suivants :
