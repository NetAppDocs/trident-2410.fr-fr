...
Oui, Trident peut être installé hors ligne. Consultez link:../trident-install/kubernetes-deploy.html["En savoir plus sur l'installation de Trident"^].
...
L'installateur Trident crée un ClusterRole Kubernetes, qui dispose d'un accès spécifique aux ressources PersistentVolume, PersistentVolumeClaim, StorageClass et Secret du cluster Kubernetes. Consultez link:../trident-install/kubernetes-customize-deploy-tridentctl.html["Personnalisez l'installation tridentctl"^].



=== Puis-je générer localement les fichiers de manifeste exacts que Trident utilise pour l'installation ?

Vous pouvez générer et modifier localement les fichiers manifestes exacts utilisés par Trident pour l'installation, si nécessaire. Consultez link:trident-install/kubernetes-customize-deploy-tridentctl.html["Personnalisez l'installation tridentctl"^].
