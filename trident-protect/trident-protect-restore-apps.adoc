...
.Étapes
. Vérifiez que la AppVault CR existe sur le cluster de destination à l'aide du plugin CLI Trident Protect :
+
[source, console]
----
...
NOTE: Si la AppVault CR n'existe pas sur le cluster de destination, créez-la en suivant les étapes dans link:trident-protect-appvault-custom-resources.html["Utilisez les objets Trident Protect AppVault pour gérer les compartiments."].

. Consultez le contenu de la sauvegarde disponible de AppVault sur le cluster de destination, et notez  `appArchivePath` de la sauvegarde que vous souhaitez restaurer :
+
[source, console]
----
...
NOTE: Lors de l'utilisation d'une CR, assurez-vous que l'espace de noms destiné à la restauration de l'application existe sur le cluster de destination.
